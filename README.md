...
`docs/specs/v1-portfolio-app/requirements.md` §4 的 AC-01…AC-10：本地 alchemy + Expo Web 全量走通；并已针对线上 URL 复验（经本机 HTTP 代理访问公网 Worker；Web 会话经同源面纱 + Cookie 翻译完成），注册/发布/点赞/我的/设置/退出全绿、控制台 0 error。Android 模拟器（Expo Go + Maestro）原生全流程已录证并打到同一线上 API：注册（`MaestroPublish`）、发布（线上 D1 id=22 + R2 图）、点赞/取消、我的、设置退出；iOS 模拟器（iPhone 17 Pro + Expo Go）已启动并打到同一线上 API（首页/详情录证）。服务端已放行 `exp://` origin，原生上传改用 `expo-file-system`。注意：直连 `*.workers.dev` 在部分网络不可达，验收需代理或自定义域名。
